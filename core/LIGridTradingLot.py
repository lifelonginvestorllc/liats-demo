...
class LIGridTradingLot(LIGridBaseLot):
    ...
    def manageLotOpenOrderTicket(self, useMarketOrder=False, boostMorePositions=False):
        ...
        if self.hasOpenPosition() and not (boostMorePositions or self.isCommandMode()):
            return False  # Abort, fully filled!

        targetAmount = targetQuantity * targetPrice
        if self.gridLotMinAmount and abs(targetAmount) < abs(self.gridLotMinAmount):
            log(f"{self.getSymbolAlias()}@{self.getLotName()}: The targetAmount={targetAmount} is too small to open an order as "
                f"{LIConfigKey.gridLotMinAmount}={self.gridLotMinAmount}", self.verbose)
            return False

        if self.gridLotMinQuantity and abs(targetQuantity) < abs(self.gridLotMinQuantity):
        ...
        if useMarketOrder:
            ...
            return needToUpdate
              ...
              (boostMorePositions or self.isCommandMode() or
        ...
        if (self.openOrderTicket and self.openOrderTicket.order_id == orderEvent.order_id) or (
            ...
            if orderEvent.status == OrderStatus.NEW or orderEvent.status == OrderStatus.SUBMITTED:
                ...
                if self.isBuyAndHoldMode():
                    self.gridTrading.dcaLastInvestedDate = getAlgoTime()
                ...
                if self.isCommandMode():
                    self.openOrderTicket = None  # Clear it to allow new orders
